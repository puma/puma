...
1. Puma will now listen on `::` by default when the host has a non-loopback IPv6 interface (i.e., it's IPv6 capable). This may introduce a breaking change if, on your host, :: binds only to IPv6 (for example, on Windows, or on systems configured for IPv6-only sockets). If you believe this is the case, set `bind 'tcp://0.0.0.0:9292'`, `port ENV.fetch('PORT', 9292), '0.0.0.0'`, or `set_default_host '0.0.0.0'` explicitly to remain IPv4 only. Review any firewall rules, health checks, deploy scripts, or host-string parsing code that assumed `0.0.0.0`, `127.0.0.1`, or unbracketed `HOST:PORT` formatting.
