...
#    @tcp = "http://127.0.0.1:#{@port}"
  ...
  def test_requests_count
    ...
    3.times { send_http_read_response GET_TEST }
    assert_equal @server.requests_count, 3
  end

  def test_trickle_attack
    socket = do_test(VALID_REQUEST, 3)
    assert_match "hello!", socket.read_body
  end
  ...
  def test_bad_client
    ...
    assert_match "Bad Request", socket.read_response
  end

  def test_bad_path
    socket = do_test("GET : HTTP/1.1\r\n\r\n", 3)
    response = socket.read_response
    assert_start_with response, "HTTP/1.1 400 Bad Request\r\nContent-Length: "
    # match is for last backtrace line, may be brittle
    assert_match(/\.rb:\d+:in [`'][^']+'\z/, response)
  end
  ...
  def test_nonexistent_http_method
    ...
    assert_match "Not Implemented", socket.read_response
  end
