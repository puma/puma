...
module TestPuma
      ...
      loop do
        begin
          ...
          when String
            ...
            ary = response.split("\r\n\r\n", 2)
            if content_length
              if ary.last.bytesize == content_length
                # STDOUT.puts "#{ary.last.bytesize} content length"
                return response
              end
            elsif chunked
              # STDOUT.puts "#{ary.last.bytesize} chunked"
              if ary.last.end_with? "\r\n0\r\n\r\n"
                # STDOUT.puts "#{ary.last.bytesize} chunked"
                return response
              end
            elsif ary.length == 2
  ...
  module Sockets
              ...
              begin
                ...
                begin
                  ...
                  if body =~ /^Hello World$/
                    ...
                    break if socket.close_connection
                  else
    ...
    def time_info(threads, clients_per_thread, time_ary, req_per_client = 1)
      ...
      digits = time_max < 100 ? 3 : (time_max < 1000 ? 2 : 1)
