...
namespace :puma do
  ...
  task :start, :roles => lambda { fetch(:puma_role) }, :on_no_matching_servers => :continue do
    ...
    run "cd #{current_path} && #{fetch(:puma_cmd)} -q -e #{puma_env} -b 'unix://#{shared_path}/sockets/puma.sock' -S #{fetch(:puma_state)} --control 'unix://#{shared_path}/sockets/pumactl.sock' 2>&1 &", :pty => false
  end
