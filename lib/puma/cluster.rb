...
module Puma
  ...
  class Cluster < Runner
    ...
    class Worker
      ...
      def ping!(status)
        @last_checkin = Time.now
        captures = status.match(/{ "backlog":(?<backlog>\d*), "running":(?<running>\d*), "pool_capacity":(?<pool_capacity>\d*), "max_threads": (?<max_threads>\d*), "processed_requests": (?<processed_requests>\d*) }/)
      ...
      Thread.new(@worker_write) do |io|
        ...
        while true
          ...
          begin
            ...
            p = server.processed_requests || 0
            payload = %Q!#{base_payload}{ "backlog":#{b}, "running":#{r}, "pool_capacity":#{t}, "max_threads": #{m}, "processed_requests": #{p} }\n!
            io << payload
          rescue IOError
            Thread.current.purge_interrupt_queue if Thread.current.respond_to? :purge_interrupt_queue
            break
